...
class MyLstm(nn.Module):
    def __init__(self, input_size=44, hidden_size=128, batch_size=1000, target_len=50, TR=0.1):
...
if __name__ == '__main__':
                ...
                for i in range(epochs):
                    ...
                    if i == epochs - 1:
                        ...
                        for batch_idx, (x_seq, x_seq_NF) in enumerate(valid_loader):
                            ...
                            loss_1s = rmse_loss(x_data_oo_torch[:, 39, :2], pred_seq_ori_torch[:, 39, :2])
                            loss_2s = rmse_loss(x_data_oo_torch[:, 49, :2], pred_seq_ori_torch[:, 49, :2])
                            loss_3s = rmse_loss(x_data_oo_torch[:, 59, :2], pred_seq_ori_torch[:, 59, :2])
                            loss_4s = rmse_loss(x_data_oo_torch[:, 69, :2], pred_seq_ori_torch[:, 69, :2])
                            loss_5s = rmse_loss(x_data_oo_torch[:, 79, :2], pred_seq_ori_torch[:, 79, :2])

                            loss = rmse_loss(x_data_oo_torch[:, 30:, :2], pred_seq_ori_torch[:, 30:, :2])
                            ...
                            rmse_5s_ = rmse_sum(x_data_oo_torch[:, 79, :2], pred_seq_ori_torch[:, 79, :2])
    ...
    np.save(file="LSTM_10_result.npy", arr=result44)
