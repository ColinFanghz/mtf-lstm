...
class MyLstm(nn.Module):
    def __init__(self, input_size=44, hidden_size=128, target_len=25, TR=0.1):
    ...
    for dataset_num in range(5):
        ...
        for tr in range(9):
            ...
            for times in range(3):
                model_name = "LSTM_ED_D{}_R{}_T{}.pkl".format(dataset_num, tr+1, times+1)
                print(model_name)
                ...
                for i in range(epochs):
                    ...
                    if i == epochs - 1:
                        ...
                        for batch_idx, (x_seq, x_seq_NF) in enumerate(valid_loader):
                            ...
                            loss_1s = mse_loss1(x_data_oo_torch[:, 19, :2], pred_seq_ori_torch[:, 19, :2]) ** 0.5
                            loss_2s = mse_loss1(x_data_oo_torch[:, 24, :2], pred_seq_ori_torch[:, 24, :2]) ** 0.5
                            loss_3s = mse_loss1(x_data_oo_torch[:, 29, :2], pred_seq_ori_torch[:, 29, :2]) ** 0.5
                            loss_4s = mse_loss1(x_data_oo_torch[:, 34, :2], pred_seq_ori_torch[:, 34, :2]) ** 0.5
                            loss_5s = mse_loss1(x_data_oo_torch[:, 39, :2], pred_seq_ori_torch[:, 39, :2]) ** 0.5
